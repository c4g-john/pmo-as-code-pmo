...
- **ENG-TC-007** (tests: ENG-AC-007): Steps: build a fixture project page, list every rendered value, and map each to its model field or repository record. Expected: no value without a source.
- **ENG-TC-008** (tests: ENG-AC-008): Steps: derive status over a charter with one past and one future dated milestone. Expected: both on the timeline, the past one marked elapsed (not done), the future one carrying its day count.
- **ENG-TC-009** (tests: ENG-AC-009): Steps: render a page with execution data containing a closed and an open feature, then re-derive the RAG with and without the execution file. Expected: real states on the rows; identical RAG either way.
- **ENG-TC-010** (tests: ENG-AC-010): Steps: author three features where the third is `after` the second, with differing story and criterion counts, then render. Expected: three layers of position, bar widths proportional to points, the formula in the legend, and no dates on the chart.
- **ENG-TC-011** (tests: ENG-AC-011): Steps: author two product requirements each `after` the other and run consistency. Expected: sequence-acyclic fails as blocking.
