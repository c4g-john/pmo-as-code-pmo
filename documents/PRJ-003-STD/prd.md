...
- **STD-PR-006** (traces: STD-BR-002): Engine continuous integration shall run the conformance suite on every change, failing on any divergence from the specification.
...
- **STD-AC-006** (verifies: STD-PR-006): Given an engine change that diverges from a specified behavior, when engine CI runs, then the conformance job fails.
