---
kind: user-story
id: ADO-user-story
project: PRJ-004-ADO
title: Launch and adoption — User Stories
owner: johntanner
status: approved
---

## Overview

Delivery slices for launch. Each requirement is a single coherent deliverable
with a distinct audience, so stories map one to one. Acceptance criteria live
with the PRD.

## User Stories

- **ADO-US-001** (traces: ADO-PR-001): As a prospective adopter, I want every released engine feature documented on the site so that evaluating PMO as Code never requires reading source code.
- **ADO-US-002** (traces: ADO-PR-002): As a skeptical reader, I want a self-governance page linking the live portfolio dashboard so that every claim on the site is verifiable within two clicks.
- **ADO-US-003** (traces: ADO-PR-003): As a sponsor, I want the launch announcements published once the engine reaches 1.0 so that the public story lands on a stable target.
- **ADO-US-004** (traces: ADO-PR-004): As a sponsor, I want a documented monthly adopter count from forks and dependents so that adoption is measured honestly instead of estimated.
- **ADO-US-007** (traces: ADO-PR-007): As a maintainer, I want prose as Markdown and repeatable content as typed data so that editing a fact once updates every surface that states it.
- **ADO-US-008** (traces: ADO-PR-008): As a reader arriving from any old link, I want the URL to still resolve so that shared links and citations never rot.
- **ADO-US-009** (traces: ADO-PR-009): As an AI or search crawler, I want the full content in clean semantic HTML at fetch time so that the site can be indexed, quoted, and cited without executing anything.
- **ADO-US-010** (traces: ADO-PR-010): As a skeptical reader, I want every claim that survived the rewrite to still be checkable so that the migration itself cannot introduce fiction.
- **ADO-US-006** (traces: ADO-PR-006): As a journalist or sharer, I want one page with verified facts, approved copy, assets, and a press contact so that I can cover or share the project accurately without asking.
- **ADO-US-005** (traces: ADO-PR-005): As a new adopter, I want a fresh template copy to reach a green gate and a live dashboard using only the README so that my first hour with the tool succeeds.
